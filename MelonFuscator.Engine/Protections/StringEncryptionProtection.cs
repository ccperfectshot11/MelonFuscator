...
public sealed class StringEncryptionProtection : IProtection
{
    ...
    public void Execute(ObfuscationContext ctx)
    {
        ...
        var cipher = ctx.Cipher;   // per-build random cipher (DynCipher)

        var holder = CilHelpers.CreateStaticHolder(module, ctx.Names.Next());
        var decrypt = EmitDecryptMethod(module, holder, cipher);
        ...
        foreach (var type in module.GetAllTypes())
        {
            foreach (var method in type.Methods)
            {
                ...
                for (int i = 0; i < instrs.Count; i++)
                {
                    ...
                    ins.Operand = Encrypt(s, cipher);            // keep ldstr, swap the literal
    ...
    private static string Encrypt(string s, ByteCipher cipher)
    {
        var bytes = System.Text.Encoding.UTF8.GetBytes(s);
        var arr = new char[bytes.Length];
        for (int i = 0; i < bytes.Length; i++)
            arr[i] = (char)cipher.EncryptByte(bytes[i], i);
        return new string(arr);
    }

    // Generates: static string Decrypt(string cipher)
    //   byte[] buf = new byte[cipher.Length];
    //   for (i) buf[i] = (byte)ByteCipher.decrypt(cipher[i], i);
    //   return Encoding.UTF8.GetString(buf);
    private static MethodDefinition EmitDecryptMethod(ModuleDefinition module, TypeDefinition holder, ByteCipher cipher)
    {
        ...
        // buf[i] = (byte)cipher.decrypt(cipher[i], i)
        n.Add(new CilInstruction(CilOpCodes.Ldloc, bufLocal));
        n.Add(new CilInstruction(CilOpCodes.Ldloc, iLocal));
        n.Add(new CilInstruction(CilOpCodes.Ldarg_0));
        n.Add(new CilInstruction(CilOpCodes.Ldloc, iLocal));
        n.Add(new CilInstruction(CilOpCodes.Callvirt, getChars));  // cipher byte on stack
        cipher.EmitDecryptByte(n, iLocal);                         // -> plaintext byte on stack
